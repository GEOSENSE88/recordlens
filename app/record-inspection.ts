...
const PROHIBITED_RULES: TextRule[] = [
  ...
  {
    // `대회`는 어떤 맥락에서도 기재할 수 없어 단어 자체를 잡는다.
    // `상` 뒤에는 조사 또는 문장부호만 허용해 `우수상태`, `장려상황` 같은 복합어 오탐을 막는다.
    expression:
      /대회|(?:최우수|우수|장려|공로)상(?=$|[\s,.;:!?·→←\-)"'’]|(?:을|를|은|는|이|가|에|과|와|도|만|의|으로|로)(?![가-힣]))|표창장|감사장|수상\s*(?:실적|사실)|입상/gu,
    label: "대회·수상",
    guidance:
      "‘대회’라는 용어는 어떤 경우에도 기재할 수 없으며, 수상경력 이외 항목에는 대회 참여·수상 사실도 기재하지 않습니다.",
    reference: "2026 기재요령 p.18, p.61 · 학교 점검 중점사항 ②",
    severity: "danger",
  },
  {
...
const INSTITUTION_ACRONYMS = [
  ...
  // `ESG`는 목록에 있었지만 탐구 서술에 워낙 흔해 사용자 지시로 뺐다.
  "ODA",
];
...
/**
 * 나이스 입력 한도를 넘는 기록을 잡는다(2026학년도 기준 모든 서술형 항목 1,500바이트).
